...
CREATE TABLE IF NOT EXISTS `notifications` (
  ...
  `related_type` varchar(50) DEFAULT NULL COMMENT '关联的业务类型（方便查询）',
  ...
  `websocket_status` tinyint DEFAULT '0' COMMENT 'WebSocket发送状态：0-未发送，1-发送中，2-发送成功，3-发送失败',
...
INSERT IGNORE INTO `email_templates` (
  `template_code`, `template_name`, `subject`, `content`, `plain_text_content`, `template_type`, `is_enabled`, `remark`
) VALUES
(
...
CREATE INDEX `idx_email_templates_search` ON `email_templates` (`template_type`, `is_enabled`, `created_at`);
...
SELECT
    ...
    cm.is_muted,
    (SELECT COUNT(*) FROM messages m WHERE m.conversation_id = c.id AND m.is_deleted = 0) as total_message_count
...
-- 所有表都包含逻辑删除字段(is_deleted)和乐观锁字段(version)
