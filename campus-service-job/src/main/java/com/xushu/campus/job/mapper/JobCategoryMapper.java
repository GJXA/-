...
public interface JobCategoryMapper extends BaseMapper<JobCategory> {
     ...
     * 查询所有启用的分类（按排序字段排序）
     */
    @Select("SELECT * FROM job_categories WHERE enabled = 1 AND deleted = 0 ORDER BY sort_order ASC")
    List<JobCategory> selectEnabledCategories();

    /**
     * 根据分类编码查询
     */
    @Select("SELECT * FROM job_categories WHERE code = #{code} AND deleted = 0")
    JobCategory selectByCode(@Param("code") String code);

    /**
     * 查询所有分类（包括禁用，按排序字段排序）
     */
    @Select("SELECT * FROM job_categories WHERE deleted = 0 ORDER BY sort_order ASC, create_time DESC")
